...
void byteSubShiftRow(unsigned char *state) {
    ...
    // Mapping of indices based on ShiftRows and S-box (s)
    int indices[16] = {0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11};

    // Apply SubBytes and ShiftRows
    for (int i = 0; i < 16; ++i) {
        tmp[i] = s[state[indices[i]]];
    }

    // Copy the result back to the state array
    for (int i = 0; i < 16; ++i) {
        state[i] = tmp[i];
    }
}
...
void inverseByteSubShiftRow(unsigned char *plainText) {
    ...
    // Mapping of indices based on inverse ShiftRows and inverse S-box (inv_s)
    int indices[16] = {0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3};

    // Apply inverse SubBytes and inverse ShiftRows
    for (int i = 0; i < 16; ++i) {
