...
string decimaltohex(int64 deci)
{
    ...
    string binary = bitset<64>(deci).to_string();
    ...
    for (unsigned int i = 0; i < binary.length(); i += 4) {
        ...
        hexstring += gethex(binary.substr(i, 4));
    }
...
void separator(string getBlock)
{
    ...
    for (unsigned int i = 0; i < getBlock.length();
        ...
        // Update the first 16 int64 Message[chunknum]
        Message[chunknum]
...
string SHA512(string myString)
{
    ...
    for (int i = 0; i < myString.size(); ++i) {
        ...
        binaryString += bitset<8>(myString[i]).to_string();
    }
    ...
    int stringLength = s1024.length();
    int tobeadded;

    // Find modded string length
    int modded = s1024.length() % 1024;

    // If modded is less than 896
    if (1024 - 128 >= modded) {
        tobeadded = 1024 - modded;
    }

    // If modded is greater than 896
    else if (1024 - 128 < modded) {
    ...
    result += decimaltohex(A);
    result += decimaltohex(B);
    result += decimaltohex(C);
    result += decimaltohex(D);
    result += decimaltohex(E);
    result += decimaltohex(F);
    result += decimaltohex(G);
    result += decimaltohex(H);
...
{
    // Input
    string S = readFromFile("SHA_512_input.txt");

    // Function Call
    cout << "The string : " << S << endl;
    freopen("Sha_output.txt","w",stdout);
    cout << "SHA-512 : " << SHA512(S);
    fclose(stdout);
    return 0;
}
