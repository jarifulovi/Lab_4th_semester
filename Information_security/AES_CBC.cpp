...
int main(){
    ...
    int v_length = 16;
    unsigned char IV[v_length]; 
    generateRandomIV(IV,v_length);
    
    // Encrypt
    for (int i = 0; i < paddedMessageLen; i += 16) {
        // XOR the ciphertext block with the previous ciphertext block
        if(i == 0){
            for(int j=0;j<16;j++){
                paddedMessage[i+j] ^= IV[i+j];
            }
        }
        else {
            for(int j=0;j<16;j++){
                paddedMessage[i+j] ^= cipher[i-16+j];
            }
        }

        // Encrypt XORed result to produce ciphertext block
    ...
    for (int i = 0; i < paddedMessageLen; i += 16) {
        ...
        // XOR decrypted block with previous ciphertext block for subsequent blocks
        if(i == 0){
            for (int j = 0; j < 16; j++) {
                decrypted[j] ^= IV[j];
            }
