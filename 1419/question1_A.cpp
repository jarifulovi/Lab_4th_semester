...
void generateRandomIV(unsigned char* iv, int length) {
    // Seed the random number generator
    srand(time(nullptr));
    
    // Generate random values for each byte of the IV
    for (int i = 0; i < length; ++i) {
        iv[i] = rand() % 256; // Generate random byte (0-255)
    }
}
...
int main() {
    ...
    string plainText;
    plainText = readFromFile("aes_input.txt");
    ...
    for (int i = 0; i < paddedMessageLen; i++) {
        paddedMessage[i] = (i < originalLen) ? plainText[i] : 0;
    }
    
    // Counter Mode: Initialization Vector (IV)
    unsigned char IV[16] = {2,4,12,53,65,34,23,45,12,88,102,112,43,34,65,81};
    ...
    for (int i = 0; i < paddedMessageLen; i += 16) {
        ...
        for (int j = 0; j < 8; ++j) {
            counterBlock[j] = (IV[j] + (i >> (j * 8))) & 0xFF;
        }
        for (int j = 8; j < 16; ++j) {
            counterBlock[j] = (i >> (j * 8)) & 0xFF;
        }

        // Encrypt Counter Block
        AESEncryption(counterBlock, expandedKey, cipher + i);

        // XOR Cipher Block with Plaintext Block
        for (int j = 0; j < 16 && i + j < paddedMessageLen; ++j) {
            cipher[i + j] ^= paddedMessage[i + j];
        }
    }
    ...
    for (int i = 0; i < paddedMessageLen; i += 16) {
        // Generate Counter Block (same as encryption)
        unsigned char counterBlock[16];
        for (int j = 0; j < 8; ++j) {
            counterBlock[j] = (IV[j] + (i >> (j * 8))) & 0xFF;
        }
        ...
        // Encrypt Counter Block
        AESEncryption(counterBlock, expandedKey, decrypted + i);

        // XOR Decrypted Block with Ciphertext Block
        for (int j = 0; j < 16 && i + j < paddedMessageLen; ++j) {
            decrypted[i + j] ^= cipher[i + j];
        }
    }
